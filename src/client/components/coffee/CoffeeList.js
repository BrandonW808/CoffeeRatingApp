...
const CoffeeList = () => {
  ...
  const fetchCoffees = async () => {
    try {
      setLoading(true);
      const response = await getCoffees({ sortBy, order: sortOrder });
  ...
  const handleAddCoffee = async (coffeeData) => {
    try {
      ...
      throw new Error(err.message || 'Failed to add coffee');
    }
  };
  ...
  // Called by CoffeeCard after any image operation
  ...
  if (loading) return <div className="loading">Loading your coffees...</div>;
        ...
        {coffees.length === 0 ? (
          ...
          coffees.map((coffee) => (
            <CoffeeCard
              ...
              onRefresh={handleRefresh}  // ← was missing
            />
          ))
        )}
      </div>
